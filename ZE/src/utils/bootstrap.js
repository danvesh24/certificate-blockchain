const config = require('../config/config');
const Organization = require('../models/organization.model');
const User = require('../models/user.model');
const { ORG_DEPARTMENT, USER_STATUS, USER_TYPE } = require('./Constants');
const { registerUser } = require('./blockchainUtils');

const ingestBootstrapData = async () => {
  const staticOrgData = [
    { name: 'superadmin', id: 1, parentId: 1 },
    { name: 'company', id: 2, parentId: 1 },
    { name: 'retailer', id: 3, parentId: 1 },
  ];
  const staticUser = [
    {
      name: 'max',
      email: 'admin33443@gmail.com',
      orgId: 1,
      password: config.commonPassword,
      department: ORG_DEPARTMENT.GENERAL,
    },
    {
      name: 'jon',
      email: 'admin33444@gmail.com',
      orgId: 2,
      password: config.commonPassword,
      department: ORG_DEPARTMENT.GENERAL,
    },
  ];
  
  // Ingest organization data
  for (let org of staticOrgData) {
    let orgData = await Organization.findOne({ id: org.id });
    if (!orgData) {
      let o = new Organization({
        id: org.id,
        name: org.name,
        parentId: org.parentId,
      });
      await o.save();
      console.log('Ingesting static org data', org.name);
    } else {
      console.log('organization already exists', org.name);
    }
  }

  // Ingest user data
  for (let user of staticUser) {
    let userData = await User.findOne({ email: user.email });
    console.log('user data is---', userData);
    if (!userData) {
      // Find the organization by orgId to get the name
      let org = await Organization.findOne({ id: user.orgId });
      if (!org) {
        console.error(`Organization with id ${user.orgId} not found.`);
        continue;
      }

      let newUser = new User({
        name: user.name,
        email: user.email,
        orgId: user.orgId,
        password: user.password,
        status: USER_STATUS.ACTIVE,
        type: USER_TYPE.ADMIN,
        department: user.department,
      });

      try {
        // Use org.name instead of org.id
        let secret = await registerUser(org.name, user.email, user.department);
        newUser.secret = secret;
        newUser.isVerified = true;
      } catch (error) {
        console.log("-----error------", error);
      }

      await newUser.save();
      console.log('----ingested static user data--', user.email);
    } else {
      console.log('user email already exists', user.email);
    }
  }
};

module.exports = { ingestBootstrapData };
